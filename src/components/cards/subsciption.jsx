  ...
  const pricingPlans = [
    {
      ...
      subtitle: "(₹ 0)",
      plan_name: "one-week",
      plan_total_price: 0, // Corrected to match free plan
      price: "Free",
      period: "",
      features: [
        "Dashboard",
    ...
    },
    {
      ...
      plan_name: "half-year",
      plan_total_price: 1, // Corrected to match subtitle
      price: "2",
      period: "per month",
      features: [
        "Dashboard",
    ...
    },
    {
      ...
      subtitle: "(₹ 1,788 / Year)",
      plan_name: "yearly",
      plan_total_price: 2, // Corrected to match subtitle
      price: "2",
      period: "per month",
      features: [
        "Dashboard",
  ...
  const SubscriptionPlan = ({ selectedPlan }) => {
    ...
    const handlePaymentSuccess = async (response, plan) => {
      try {
        ...
        if (result.data.success) {
          ...
          router.push(
            `/payment-success?reference=${result.data.reference}&plan=${result.data.plan}`
          );
        } else {
    ...
    const handleCheckout = async (amount, plan) => {
      ...
      try {
        ...
        if (!key) throw new Error("Failed to retrieve payment key.");
                ...
                >
                  ...
                  <CardFooter>
                    <Button
                      ...
                      onClick={() => handleCheckout(plan.plan_total_price, plan.plan_name)}
                    >
