...
export default function JournalAnalysis() {
  ...
  const getInitialState = () => {
    ...
    return {
      journalsDateRange: {
        from: savedFilters.journalsFrom
          ? parseDateString(savedFilters.journalsFrom)
          : startOfMonth(new Date()),
        to: savedFilters.journalsTo
          ? parseDateString(savedFilters.journalsTo)
          : endOfMonth(new Date()),
      },
      filters: {
        minWinRate:
          Number(savedFilters.minWinRate) ||
          Number(searchParams.get("minWinRate")) ||
          defaultFilters.minWinRate,
        maxWinRate:
          Number(savedFilters.maxWinRate) ||
          Number(searchParams.get("maxWinRate")) ||
          defaultFilters.maxWinRate,
        minTrades:
          Number(savedFilters.minTrades) ||
          Number(searchParams.get("minTrades")) ||
          defaultFilters.minTrades,
        maxTrades:
          Number(savedFilters.maxTrades) ||
          Number(searchParams.get("maxTrades")) ||
          defaultFilters.maxTrades,
        minRulesFollowed:
          Number(savedFilters.minRulesFollowed) ||
          Number(searchParams.get("minRulesFollowed")) ||
          defaultFilters.minRulesFollowed,
        maxRulesFollowed:
          Number(savedFilters.maxRulesFollowed) ||
          Number(searchParams.get("maxRulesFollowed")) ||
          defaultFilters.maxRulesFollowed,
      },
      pagination: {
        currentPage:
          Number(savedFilters.page) || Number(searchParams.get("page")) || 1,
        totalPages: 1,
        totalItems: 0,
        limit: 12,
      },
  ...
  const [journalsDateRange, setJournalsDateRange] = useState(
    initialState.journalsDateRange
  );
  ...
  const updateURLAndCookies = () => {
    ...
    if (journalsDateRange.from)
      params.set(
        "journalsFrom",
        formatDateWithoutTimezone(journalsDateRange.from)
      );
    else params.delete("journalsFrom");
    if (journalsDateRange.to)
      params.set("journalsTo", formatDateWithoutTimezone(journalsDateRange.to));
    else params.delete("journalsTo");

    if (filters.minWinRate !== defaultFilters.minWinRate)
      params.set("minWinRate", filters.minWinRate.toString());
    else params.delete("minWinRate");
    if (filters.maxWinRate !== defaultFilters.maxWinRate)
      params.set("maxWinRate", filters.maxWinRate.toString());
    else params.delete("maxWinRate");
    if (filters.minTrades !== defaultFilters.minTrades)
      params.set("minTrades", filters.minTrades.toString());
    else params.delete("minTrades");
    if (filters.maxTrades !== defaultFilters.maxTrades)
      params.set("maxTrades", filters.maxTrades.toString());
    else params.delete("maxTrades");
    if (filters.minRulesFollowed !== defaultFilters.minRulesFollowed)
      params.set("minRulesFollowed", filters.minRulesFollowed.toString());
    else params.delete("minRulesFollowed");
    if (filters.maxRulesFollowed !== defaultFilters.maxRulesFollowed)
      params.set("maxRulesFollowed", filters.maxRulesFollowed.toString());
    else params.delete("maxRulesFollowed");
    if (pagination.currentPage > 1)
      params.set("page", pagination.currentPage.toString());
    else params.delete("page");

    router.push(`/performance-analytics?${params.toString()}`, {
      scroll: false,
    });
    Cookies.set(
      "journalAnalysisFilters",
      JSON.stringify({
        journalsFrom: journalsDateRange.from
          ? formatDateWithoutTimezone(journalsDateRange.from)
          : null,
        journalsTo: journalsDateRange.to
          ? formatDateWithoutTimezone(journalsDateRange.to)
          : null,
        ...filters,
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/journals/filters`,
          {
            params: {
              startDate: formatDateWithoutTimezone(
                journalsDateRange.from || startOfMonth(new Date())
              ),
              endDate: formatDateWithoutTimezone(
                journalsDateRange.to || endOfMonth(new Date())
              ),
              page: pagination.currentPage,
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setMonthlyJournals(
        Object.keys(response.data.data).length === 0 ? null : response.data.data
      );
      setPagination((prev) => ({ ...prev, ...response.data.pagination }));
    } catch (err) {
      toast({
  ...
  const isWinRateModified =
    filters.minWinRate !== defaultFilters.minWinRate ||
    filters.maxWinRate !== defaultFilters.maxWinRate;
  const isTradesModified =
    filters.minTrades !== defaultFilters.minTrades ||
    filters.maxTrades !== defaultFilters.maxTrades;
  ...
  return (
    <div className="p-4 sm:p-6 bg-background rounded-b-xl">
      <div className="bg-card shadow-md border border-border p-4 rounded-xl min-h-[50vh]">
          ...
          <h2 className="text-lg sm:text-xl font-semibold">
            ...
            {journalsDateRange.from && journalsDateRange.to && (
              <span className="ml-2 text-muted-foreground font-normal text-xs sm:text-sm">
                ({format(journalsDateRange.from, "MMM dd, yyyy")} -{" "}
                {format(journalsDateRange.to, "MMM dd, yyyy")})
              </span>
            )}
          </h2>
        ...
        {error && (
          <div
            className="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded mb-4 text-xs sm:text-sm"
            role="alert"
          >
            {error}
          </div>
                  ...
                  {Object.entries(monthlyJournals).map(([date, journal]) => (
                    <div key={date} onClick={() => handleCardClick(date)}>
                      <JournalCard
                        ...
                        onDelete={handleDelete} // <-- new handler
